...
# --- ADVANCED SCREENER SCRAPING ENGINE (WITH LOGIN) ---
def get_fundamentals(symbol):
    clean_sym = symbol.upper().replace(".NS", "").strip()
    url_login = "https://www.screener.in/login/"
    url_company = f"https://www.screener.in/company/{clean_sym}/"
    ...
    data = {
        ...
        "sales_growth": 0.0, "sales_growth_3y": 0.0, 
        "profit_growth": 0.0, "profit_growth_3y": 0.0, 
        "opm": 0.0, "interest_cov": 0.0,
        "promoter_hold": 0.0, "promoter_pledge": 0.0, 
        "fii_hold": 0.0, "dii_hold": 0.0,
        "sector": "N/A", "cap_size": "N/A"
    }
    
    session = requests.Session()
    try:
        # Step 1: Get CSRF Token
        r = session.get(url_login, headers=headers, timeout=10)
        ...
        if csrf_token:
            ...
            session.post(url_login, data=login_data, headers=headers, timeout=10)
        
        # Step 3: Fetch custom data directly from the top section
        resp = session.get(url_company, headers=headers, timeout=10)
            ...
            for li in ratio_items:
                ...
                if name and val:
                    ...
                    if matches:
                        ...
                        elif 'roe' in n_text and '3' not in n_text: data["roe"] = v_num
                        elif 'debt to equity' in n_text: data["debt_eq"] = v_num
                        elif 'sales growth' in n_text and '3' not in n_text: data["sales_growth"] = v_num
                        elif 'sales growth 3years' in n_text or 'sales growth 3 yrs' in n_text: data["sales_growth_3y"] = v_num
                        elif 'profit growth' in n_text and '3' not in n_text: data["profit_growth"] = v_num
                        elif 'profit var 3yrs' in n_text or 'profit growth 3years' in n_text: data["profit_growth_3y"] = v_num
                        elif 'opm' in n_text: data["opm"] = v_num
                        elif 'int. coverage' in n_text or 'interest coverage' in n_text: data["interest_cov"] = v_num
                        ...
                        elif 'fii holding' in n_text: data["fii_hold"] = v_num
                        elif 'dii holding' in n_text: data["dii_hold"] = v_num
...
• <b>Sales Growth (TTM / 3Y):</b> {fund['sales_growth']}% / {fund['sales_growth_3y']}% [Target: &gt; 10%]{sales_chk}

• <b>Profit Growth (TTM / 3Y):</b> {fund['profit_growth']}% / {fund['profit_growth_3y']}% [Target: &gt; 12%]{profit_chk}
