...
import bearish_fundamental

# -------------------------------------------------------------
# TELEGRAM CONFIGURATION
# -------------------------------------------------------------
BOT_TOKEN = "8714236791:AAE1wY-oPY5gvtdvcxwZsbRbdtMI64wtnsg"
CHAT_ID = "8803628876"
...
# -------------------------------------------------------------
# 1. CHARTINK SCREENERS LIST (10 SCANNERS UPDATED)
# -------------------------------------------------------------
SCREENS = [
    {
        "name": "BEARISH 20-Day Low Volume Bounce",
        "url": "https://chartink.com/screener/bearish-20-day-low-volume-bounce",
    },
    {
        "name": "BEARISH RSI Double Bottom GK",
        "url": "https://chartink.com/screener/bearish-rsi-double-bottom-gk",
    },
    {
        "name": "BEARISH RSI Stochastic Sniper GK",
        "url": "https://chartink.com/screener/rsi-stochastic-sniper-gk",
    },
    {
        "name": "BEARISH Bollinger Bottom Bounce GK",
        "url": "https://chartink.com/screener/bollinger-bottom-bounce-gk",
    },
    {
        "name": "BEARISH 200 EMA Pullback Swing GK",
        "url": "https://chartink.com/screener/200-ema-pullback-swing-gk",
    },
    {
        "name": "INSTITUTIONAL 200 EMA BOUNCE RADAR GK",
        "url": "https://chartink.com/screener/institutional-200-ema-bounce-radar-gk",
    },
    {
        "name": "OVERSOLD RSI HAMMER REVERSAL RADAR GK",
        "url": "https://chartink.com/screener/oversold-rsi-hammer-reversal-radar-gk",
    },
    {
        "name": "QUALITY 50 EMA PULLBACK RADAR GK",
        "url": "https://chartink.com/screener/quality-50-ema-pullback-radar-gk",
    },
    {
        "name": "LOWER BOLLINGER BAND REBOUND RADAR GK",
        "url": "https://chartink.com/screener/lower-bollinger-band-rebound-radar-gk",
    },
    {
        "name": "3-DAY DIP CLIMAX RECOVERY RADAR GK",
        "url": "https://chartink.com/screener/3-day-dip-climax-recovery-radar-gk",
    },
]
...
def calculate_adx(df, period=14):
    try:
        ...
        plus_di = 100 * pd.Series(plus_dm, index=df.index).ewm(alpha=1/period, adjust=False).mean() / (tr_smoothed + 1e-9)
        minus_di = 100 * pd.Series(minus_dm, index=df.index).ewm(alpha=1/period, adjust=False).mean() / (tr_smoothed + 1e-9)
    ...
    try:
        ...
        if table:
            ...
            for i, h in enumerate(headers):
                if any(k in h for k in ["nse", "symbol", "stock"]):
                    sym_idx = i
                elif any(k in h for k in ["price", "close"]):
                    price_idx = i
                elif any(k in h for k in ["chg", "change"]):
                    chg_idx = i
                elif any(k in h for k in ["volume", "vol"]):
                    vol_idx = i
...
def generate_stock_card(symbol, hits_count):
    try:
        ...
        adx_val = round(calculate_adx(clean_df), 1)
        ...
        if v20 > v50 > v200:
            ema_str = f"20 {s1} 50 {s2} 200 EMA (🟢 UPTREND DIP)"
        elif v20 < v50 < v200:
        ...
        f_data = bearish_fundamental.get_fundamental_analysis(clean_sym)
...
def run_full_stock_radar():
    ...
    init_msg = (
        "<b>MY STOCK RADAR (BOTTOM FISHING):</b>\n"
        f"🕒 <b>Scan: {now_ist}</b>\n"
        "=============================="
    )
    ...
    with sync_playwright() as p:
        ...
        for index, screen in enumerate(SCREENS, start=1):
            ...
            if not stocks:
                block_text = f"<blockquote>🔬 <b>[{index}/{total_screens}] {screener_name}</b> | <a href='{page_url}'>[📊 Screener]</a>\n⚪ <b>0 Stocks Found</b></blockquote>"
            else:
                block_text = f"<blockquote expandable>🔬 <b>[{index}/{total_screens}] {screener_name}</b> | <a href='{page_url}'>[📊 Screener]</a>\nTotal Stocks: {len(stocks)}\n\n"
    ...
    for s in analyzed_stocks:
        ...
        # Master Filters: RSI < 40, RVOL >= 1.5, Price > EMA200
        pass_rsi = (rsi < 48.0)
        pass_rvol = (rvol >= 1.5)
        pass_ema = (price >= (v200 * 0.98))
    ...
    sweet_spot = [s for s in filtered_stocks if 0.5 <= s['change_pct'] <= 4.99]
    ...
    main_header = (
        ...
        "==============================\n"
        "🎯🎯 <b>BOTTOM FISHING / DIP BUYING PICKS</b> 🎯🎯\n"
        "=============================="
    )
    ...
    dispatch_zone_cards("SWEET SPOT ZONE", "0.5%", "4.99%", sweet_spot)
