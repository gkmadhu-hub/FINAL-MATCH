...
def generate_stock_card(symbol, hits_count):
    try:
        ...
        try:
            pledge_val_num = float(pledge_raw or 0.0)
        except Exception:
            pledge_val_num = 0.0
...
# -------------------------------------------------------------
# 5. MAIN EXECUTOR (MASTER FILTER — BULLISH 11-SCANNER SYSTEM)
# -------------------------------------------------------------
def run_full_stock_radar():
    ...
    for s in analyzed_stocks:
        ...
        roce = s.get('roce', 0.0)
        ic = s.get('ic', 0.0)
        piotroski = s.get('piotroski', 0)
        pledge = s.get('pledge', 0.0)
        industry = str(s.get('industry', '')).lower()
        ...
        pass_pio = (piotroski >= 5)

        # Promoter Pledge <= 5%
        pass_pledge = (pledge <= 5.0)

        # 3. BANKING / FINANCIAL STOCK EXCEPTION
        is_financial = any(
            k in industry
            for k in [
                'bank',
        ...
        if is_financial:
            # Banking / Financial stocks: D/E and Interest Coverage are not applied
            pass_fund_metrics = (roce >= 15.0)
        else:
        ...
        # 4. FINAL FUNDAMENTAL FILTER
        pass_fund = (
            pass_fund_metrics and
            pass_pio and
            pass_pledge
        )

        # 5. FINAL MASTER FILTER
        if (
            pass_rsi and
            pass_rvol and
            pass_ema and
            ...
            print(
                f"❌ REJECTED {sym} "
                f"(RSI:{pass_rsi}, RVOL:{pass_rvol}, "
                f"EMA:{pass_ema}, FUND:{pass_fund})"
            )
